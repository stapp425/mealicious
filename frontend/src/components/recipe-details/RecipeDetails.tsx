...
const RecipeDetails: React.FC = () => {
  ...
  useEffect(() => {
    const scrollButton = document.getElementById("scroll-button") as HTMLElement
    if(!lg)
      scrollButton.style.bottom = "100px"
    else
      scrollButton.removeAttribute("style")

    return () => scrollButton.removeAttribute("style")
  }, [lg])
  
  return (
    <RecipeDetailsContext.Provider value={{data: recipe, setData: setRecipe}}>
      <Container className="mx-auto lg:flex">
        {!lg && <Options printContent={contentRef.current as HTMLDivElement} className="w-full fixed bottom-0 left-0 shadow-options"/>}
        <div ref={contentRef} className="lg:w-[50vw] lg:max-w-[1000px] space-y-6 pb-[85px] lg:pb-0 *:print:p-6 *:print:max-w-none *:print:w-full">
        ...
        {
          lg &&
          <div className="block w-1/4 max-w-[300px] h-fit sticky top-[24px] left-0 border border-slate-400 mt-6 rounded-md">
            <Sections
