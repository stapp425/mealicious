  ...
  function sortRecipes(sort: RecipeSort) {
    switch(sort) {
      ...
      case "calories":
        ...
        break
      case "time":
        setData((d: RecipeType[]) => [...d].sort((a: RecipeType, b: RecipeType) => a.times.prepTime - b.times.prepTime))
    }
  }
    ...
    <ActiveRecipeContext.Provider value={activeRecipe.title}>
      <div className="relative flex-1 grid grid-cols-[33vw_1fr] xl:grid-cols-[1fr_33vw]">
        <div className="h-full p-4 flex flex-col gap-4 border border-r-slate-300">
          <h1 className="font-bold text-4xl">All Recipes</h1>
          <div className="flex justify-between gap-4 w-full">
            <Search/>
            <Select onValueChange={sortRecipes}>
              <SelectTrigger className="h-[35px] xl:h-[50px] w-[180px] rounded-full">
        ...
        <div className="flex items-center col-start-2 size-full p-4">
          <div className="h-[80vh] size-full grid grid-rows-2 grid-cols-2 gap-4 p-4 rounded-lg bg-white">
            { 
              isFirstRender
                ? <div className="text-slate-500 row-span-2 col-span-2 flex flex-col justify-center items-center gap-3">
                    <Clipboard size={96}/>
