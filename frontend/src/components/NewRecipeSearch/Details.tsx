...
export function DesktopDetails({ recipe }: Props): React.ReactElement {
  const { toast } = useToast()
  const currentUser = useContext<CurrentUser>(UserContext)
  const mealsCollectionRef = useRef<CollectionReference>(collection(firestore, "meals"))

  async function saveRecipe(recipe: Recipe) {
    try {
      await addDoc(mealsCollectionRef.current, { ...recipe, userId: currentUser?.uid })
      toast({
        title: "Success!",
...
export function MobileDetails({ recipe }: Props): React.ReactElement {
  const { toast } = useToast()
  const currentUser = useContext<CurrentUser>(UserContext)
  const mealsCollectionRef = useRef<CollectionReference>(collection(firestore, "meals"))
