...
const Image: React.FC<ImageProps> = ({ className, name = "image", register, error, setValue, imageState, ...props }) => {
  ...
  return (
    <div
      {...props}
      className={cn(`h-[350px] xl:h-auto xl:max-h-none flex flex-col overflow-hidden relative group bg-slate-200 rounded-md`, className)}
    >
      <Input
        {
          ...register(name, {
      ...
      />
      {
        ...
        : 
          <div className="size-full flex flex-col justify-between gap-[100px] p-4">
            <Placeholder
              icon={<Plus size={64}/>}
              className="flex-1 relative top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center items-center gap-2 bg-slate-200 text-lg text-slate-500 font-bold rounded-md"
            >
              <Placeholder.Action type="button" onClick={() => addImageButton.current?.click()} className="text-base">Add an Image</Placeholder.Action>
            </Placeholder>
            {
              error.image &&
              <Error.Label className="z-10">
                {error.image.message}
              </Error.Label>
            }
          </div>
