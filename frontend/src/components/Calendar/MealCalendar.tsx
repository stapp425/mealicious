...
import { defaultPlan, formatPlans, type Plan } from "@/types/plan"
...
const MealCalendar: React.FC = () => {
  ...
  const { data: plans, setData: setPlans } = useFirestoreFetch<Plan>(createQuery(user as User, "plans"))
  const { data: meals } = useFirestoreFetch<Meal>(createQuery(user as User, "meals"))
    ...
    <div className="relative min-h-[calc(100vh-150px)] flex flex-col items-start gap-2 bg-orange-100">
      <div className="flex flex-col h-[calc(100vh-150px)] min-w-[450px] bg-white px-6 py-4 mx-auto space-y-4">
        ...
        { 
          ...
          ? <Calendar plans={formatPlans(plans)} className="flex-1"/>
          : <Placeholder.Root
              icon={<X size={64}/>}
              className="flex-1"
            >
